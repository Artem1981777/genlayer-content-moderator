...
async function scenarioRewardAppeal(){
  console.log("===== SCENARIO A: reporter-reward REMOVE + completed appeal =====");
  const rec={label:"reward_appeal"};
  try{
    const c=await createAndGetId(); const id=c.id; rec.item_id=id; txs.A_create=c.tx;
    console.log("  item_id:",id);
    txs.A_ingest=await w(cAuthor,"ingest",[id,SCAM_URL],MIN_STAKE);
    await waitStatus(id,["ingested"],20);
    txs.A_report=await w(cOp,"report",[id],REPORT_BOND);
    txs.A_moderate=await w(cOp,"moderate",[id],0n);
    ...
    console.log("  VERDICT:",it.verdict,"| conf:",it.confidence);
    txs.A_enforce=await w(cOp,"enforce",[id],0n);
    ...
    if(it.verdict==="REMOVE"||it.verdict==="FLAG"){
      txs.A_appeal=await w(cAuthor,"appeal",[id,"Author: I believe this was wrongly flagged, requesting human re-review."],APPEAL_BOND);
      txs.A_resolve=await w(cOp,"resolve_appeal",[id],0n);
      it=await readJson(cOp,"get_item",[id]);
      rec.appeal_outcome=it.appeal_outcome; rec.verdict_after=it.verdict; rec.stake_outcome=it.stake_outcome;
      console.log("  APPEAL:",it.appeal_outcome,"| verdict:",it.verdict,"| stake_outcome:",it.stake_outcome);
    }
    rec.ok=true;
  }catch(e){ rec.ok=false; rec.error=String(e?.message||e); console.log("  SCENARIO A FAILED:",rec.error); }
  summary.push(rec); return rec;
}
async function scenarioFlag(){
  console.log("===== SCENARIO B: FLAG (mild borderline) =====");
  const rec={label:"flag_mild"};
  try{
    const c=await createAndGetId(); const id=c.id; rec.item_id=id; txs.B_create=c.tx;
    console.log("  item_id:",id);
    txs.B_ingest=await w(cAuthor,"ingest",[id,MILD_URL],MIN_STAKE);
    await waitStatus(id,["ingested"],20);
    txs.B_moderate=await w(cOp,"moderate",[id],0n);
    await waitStatus(id,["moderated"],20);
    let it=await readJson(cOp,"get_item",[id]);
    ...
    txs.B_enforce=await w(cOp,"enforce",[id],0n);
    it=await readJson(cOp,"get_item",[id]);
    rec.stake_outcome=it.stake_outcome; rec.enforcement=it.enforcement_action;
    console.log("  ENFORCED:",it.enforcement_action,"| stake_outcome:",it.stake_outcome);
    rec.ok=true;
  }catch(e){ rec.ok=false; rec.error=String(e?.message||e); console.log("  SCENARIO B FAILED:",rec.error); }
  summary.push(rec); return rec;
}
async function main(){
  ...
  writeFileSync("registry-seed-finish.json",JSON.stringify({address:ADDRESS,network:"bradbury",explorer:"https://explorer-bradbury.genlayer.com",summary:summary,txs:txs,payouts:payouts,total:all.total,config:config},null,2));
